...
{ 
	...
	ws.send("SET"+ " set_center_freq="+123456 );
...
{ 
	...
	{
		case "up5k":
				demodulator_set_offset_frequency(0, demodulators[0].offset_frequency + 5000);
			break;

		case "up1k":
				demodulator_set_offset_frequency(0, demodulators[0].offset_frequency + 1000);
			break;
		case "up10":
...
function demodulator_digital_replace(subtype)
{
    ...
    case "rtty":
        ...
        demodulator_analog_replace('am', true);
        demodulator_buttons_update();
        break;
    }
...
function secondary_demod_push_data(x)
{
	...
	for(x=0;x<w;x++)
	{
		var color=0x56432+x*100; //waterfall_mkcolor(data[x]+secondary_demod_fft_offset_db);
